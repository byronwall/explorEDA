...
The strongest existing path is scalar data → field summary → charts and rows → linked filters. Saved workspace structures and a host callback exist. Durable saved views, reusable transformed tables, and mark-level provenance do not exist.
...
Profiles drive table filters and initial chart choices. They are not a source schema: users cannot correct an inferred type, declare units, set a canonical alias, or provide a field description. Other chart and color paths still perform their own conversions. Thus one profile does not guarantee identical interpretation everywhere.
...
Expansion displays a panel in an overlay sized to the window. Escape closes it. Expansion applies to the whole panel; individual facet cells cannot be expanded or pinned separately.
...
Histogram bins, category counts, pivot aggregates, box statistics, violin densities, and line reduction already transform data. These computations belong to chart code. They are not named, inspectable, reusable nodes in a common dataflow system.

There are no user-defined group tables, joins, pivots-as-sources, regression tables, confidence bands, principal components, or reusable density outputs. Group, rank, and advanced expression shapes in types do not have corresponding active evaluator support.

Sources: [calculation UI][calcui], [form][calcform], [parser][parser], [evaluator][calculator], [calculation manager][calcstate], [function registry][functions], [data provider][provider].
...
Groups come from the full loaded source. They do not disappear merely because another chart filters out their records. Wrap order follows group discovery. Grid labels use lexical sorting. There is no user-defined order or visibility subset.

Wrap cells have minimum height around 230 pixels and target minimum width around 260 pixels. Grid cells enforce minimum dimensions around 220 by 160 pixels. Overflow scrolls within the facet container. A large faceted chart can therefore require internal scrolling.
...
The facet header states the scale population and cross-facet selection scope. Field changes recompute their source extents.

All cells edit the parent chart's filters. A scatter brush selects its X/Y range across the dataset. It does not automatically add the row and column facet keys. There is no switch between “this facet only” and “all facets.” Facet headers are labels, not filter controls.
...
There is no shared formatting model for currency, percentages, units, scientific notation, or date granularity. A chart label can manually include a unit, but that is text rather than field metadata.

### Color scales

Charts can refer to a shared color scale by ID. Automatic scale creation commonly reuses a scale named after a field. Numerical palettes include Viridis, Inferno, Magma, Plasma, Warm, and Cool. Categorical choices include Category10 and Set3, plus individual category colors.
...
Automatic color typing requires actual finite numbers for a numeric scale; mixed source types use categories. It can disagree with the field profile. Numerical bounds are set when a scale is created; they are not a declared all/working/visible population policy.

A Color Legend panel can explain and filter categorical values. It is not automatically added for every color mapping. Line series have their own palette and legend system. Legend lookup by field name can diverge from a chart that uses a differently named scale ID.
...
Color applies to bins or categories. This is not a stacked grouping feature. There is no sum/average measure, pre-aggregated-value mode, stacked bar, grouped bar, or percentage bar mode.
...
Large series use width-related reduction that preserves representative first/minimum/maximum points within buckets. This path sorts retained points by X. Smaller series retain input order, so unsorted input can produce a path that doubles back. Constant-X reduction also needs care because the reduction path can collapse data sharply.
...
Quartiles use interpolated positions. The box shows the middle half and median. Whisker choices are Tukey-style fences, minimum/maximum, or mean ± two population standard deviations. The current Tukey branch clamps theoretical fences to the overall minimum/maximum. It does not choose the extreme observed points inside the fences.
...
The beeswarm uses deterministic sampling up to 1,000 points per group and a bounded placement search. Its collision calculation mixes horizontal pixels with vertical data units. It is a heuristic overlay, not a guarantee of collision-free screen-space packing.
...
The data hook reads an optional size field, but the active points material uses one uniform point size. Missing coordinates become zero; the geometry also replaces falsy/non-numeric results with zero. This differs from the two-dimensional scatter's omission of invalid points.
...
Nullish cells show a dash. Failed calculations show Error, with a reason in the cell title. Booleans show Yes/No. Numbers align right. Other values align left. Cells use a compact single-line presentation with a title for the raw value. There are no aliases, currency/date presets, rich cells, detail panels, inline editing, row bookmarks, or conditional formatting rules.
...
The separate Rows mode adds local filters after global chart filtering. These settings remain separate from dashboard serialization. Its column list includes newly added calculations.
...
Categorical values retain source types. Quoted labels distinguish ambiguous text values. The legend resolves scales by field name, so manually named scales can diverge from a chart's chosen scale ID.
...
The callback compares a fingerprint of charts, calculations, colors, and grid settings. Cache-only changes do not cause a new emitted state when that fingerprint stays equal. Prop-driven restoration suppresses feedback emissions.
...
The 10,000-row sample passed a desktop browser walkthrough. There was no size sweep, memory profile, frame-rate test, or long-session stability run. Existing bundle checks concern import composition, not interactive scalability.
